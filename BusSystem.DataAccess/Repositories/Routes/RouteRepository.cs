...
public class RouteRepository : Repository<int, BusSystem.Core.Routes.Route>
{
    ...
    {
        var tiempoHoras = (newRouteDto.Distance / 80.0) * 1.4;
        var tiempo = TimeSpan.FromHours(tiempoHoras);
    ...
    {
        var tiempoHoras = (routeDto.Distance / 80.0) * 1.4;
        var tiempo = TimeSpan.FromHours(tiempoHoras);
        var places = Context.Places
